...
\begin{align}
	0=\frac{\partial}{\partial a_{i}}\mathcal{C}[m_{\mathcal{D}}]&=-2\sum_{k=k_{-}(y_{i})}^{k_{+}(y_{i})-1}{\alpha_{i,k}\int\limits_{x_{k}}^{x_{k+1}}{(q-a_{i}(x))g(q)dq}}\label{foc_a}\\
	0=\frac{\partial}{\partial x_{j}}\mathcal{C}[m_{\mathcal{D}}]&=\sum_{i=j-1-M}^{j-2}{\alpha_{i,j-1}(x_{j}-a_{i}(x))^{2}g(x_{j})}-\sum_{i=j-M}^{j-1}{\alpha_{i,j}(x_{j}-a_{i}(x))^{2}g(x_{j})}
\end{align}
...
In what follows, suppose that $f(e)=N\mathbf{1}_{-\bar{e}\leq e\leq \bar{e}}$. Equation \ref{eq:alpha} becomes
\begin{align}
	...
	&=\frac{1}{M}\mathbf{1}_{j-M\leq i<j}
\end{align}

% ---------------------------------------------------------------------------- %
% uniformly distributed error
\subsubsection{Tiny Example}
Suppose that $M=N=1$. There is one breakpoint, $x_{1}$, two actions, $a_{-1}$ and $a_{0}$. 
\begin{align}
...
\end{align}
According to Equation \ref{eq:G_map},
\begin{equation}
	a_{0}=\frac{x_{1}}{2}
\end{equation}
Should be 
