...
\end{equation}
Let $k_{+},k_{-}:[-\bar{e},1+\bar{e}]\rightarrow\{0,\ldots,K\}$ be given by
\begin{align}
...
\begin{align}
	...
	q_{-}(\widetilde{m})&=x_{k_{-}(\widetilde{m})}
\end{align}
The receiver takes the action 
\begin{equation}
	a(\widetilde{m};a,b)=\frac{I(\widetilde{m};a,b,1)}{I(\widetilde{m};a,b,0)}
\end{equation}
There are 
\begin{equation}
...
\begin{align}
	...
	&=\sum_{i=-M}^{K-1}{\int\displaylimits_{y_{i}}^{y_{i+1}}{\left[\sum_{j=\max\{0,i+1\}}^{\min\{i+M,K-1\}}{\int\displaylimits_{x_{j}}^{x_{j+1}}{(q-a_{i})^{2}f\left(\widetilde{m}-\tfrac{j}{K}\right)g(q)dq}}\right]d\widetilde{m}}}\\
	&=\sum_{i=-M}^{K-1}{\sum_{j=\max\{0,i+1\}}^{\min\{i+M,K-1\}}{\left[\int\displaylimits_{y_{i}}^{y_{i+1}}{\:\int\displaylimits_{x_{j}}^{x_{j+1}}{(q-a_{i})^{2}f\left(\widetilde{m}-\tfrac{j}{K}\right)g(q)dq}}d\widetilde{m}\right]}}
\end{align}
from which it follows (via a first-order condition) that
\begin{equation}
	a_{i}(x)=\frac{\mathcal{I}_{i,1}(x)}{\mathcal{I}_{i,0}(x)}
\end{equation}
where
\begin{equation}
	\mathcal{I}_{i,k}(x)=\sum_{j=\max\{0,i+1\}}^{\min\{i+M,K-1\}}{\left[\int\displaylimits_{y_{i}}^{y_{i+1}}{\:\int\displaylimits_{x_{j}}^{x_{j+1}}{q^{k}f\left(\widetilde{m}-\tfrac{j}{K}\right)g(q)dq}}d\widetilde{m}\right]}
\end{equation}
and $x=(x_{0},\ldots,x_{K+1})$. Now one may alternative write $C[m_{\mathcal{D}}]$ as
\begin{align}
	C[m_{\mathcal{D}}]&=\sum_{j=0}^{K-1}{\sum_{i=j-M}^{j-1}{\left[\int\displaylimits_{y_{i}}^{y_{i+1}}{\:\int\displaylimits_{x_{j}}^{x_{j+1}}{(q-a_{i}(x))^{2}f\left(\widetilde{m}-\tfrac{j}{K}\right)g(q)dq}}d\widetilde{m}\right]}}
\end{align}
so that for $j\in\{1,\ldots,K\}$,
\begin{align}
	\frac{\partial }{\partial x_{j}}C[m_{\mathcal{D}}]&=-\sum_{i=j-M}^{j-1}{\left[\int\displaylimits_{y_{i}}^{y_{i+1}}(x_{j}-a_{i}(x))^{2}f\left(\widetilde{m}-\tfrac{j}{K}\right)g(x_{j})d\widetilde{m}\right]}\notag\\
	&\hspace{.5in}-2\sum_{i=j-M}^{j-1}{\left[\int\displaylimits_{y_{i}}^{y_{i+1}}{\:\int\displaylimits_{x_{j}}^{x_{j+1}}{(q-a_{i}(x))f\left(\widetilde{m}-\tfrac{j}{K}\right)g(q)dq}}d\widetilde{m}\right]}\frac{\partial a_{i}}{\partial x_{j}}
\end{align}
In the special case that 
\begin{align}
	f(e)&=(2\bar{e})^{-1}\chi_{[-\bar{e},\bar{e}]}(x)\\
	g(x)&=\chi_{[0,1]}(x)
\end{align}
we obtain
\end{document}
