...
\usepackage{amsmath,amsfonts,fullpage,setspace}
\begin{document}
\onehalfspacing
...
Fix an integer $M\geq1$ and define $K=M\times N$. Define $d=\frac{1}{2K}$. Consider the partition
...
\begin{align}
	...
	&=\sum_{i=-M}^{K-1}{\int\displaylimits_{2\bar{d}i}^{2\bar{d}(1+i)}{\left[\sum_{j=k_{-}(2\bar{d}i)}^{k_{+}(2\bar{d}i)-1}{\int\displaylimits_{x_{j}}^{x_{j+1}}{(q-a_{i})^{2}f\left(\widetilde{m}-\tfrac{j}{K}\right)g(q)dq}}\right]d\widetilde{m}}}\\
	&=\sum_{i=-M}^{K-1}{\int\displaylimits_{2\bar{d}i}^{2\bar{d}(1+i)}{\left[\sum_{j=\max\{0,i-M\}}^{\min\{i+M,K\}-1}{\int\displaylimits_{x_{j}}^{x_{j+1}}{(q-a_{i})^{2}f\left(\widetilde{m}-\tfrac{j}{K}\right)g(q)dq}}\right]d\widetilde{m}}}\\
	&=\sum_{i=-M}^{K-1}{\sum_{j=\max\{0,i-M\}}^{\min\{i+M,K\}-1}{\left[\int\displaylimits_{2\bar{d}i}^{2\bar{d}(1+i)}{\int\displaylimits_{x_{j}}^{x_{j+1}}{(q-a_{i})^{2}f\left(\widetilde{m}-\tfrac{j}{K}\right)g(q)dq}}\right]d\widetilde{m}}}
\end{align}
There are 
\begin{equation}
	\frac{1+2\bar{e}}{2\bar{d}}=M+K
\end{equation}
unique actions (corresponding to $M+K$ subintervals of $[-\bar{e},1+\bar{e}]$). Note that 
\begin{align}
	k_{+}(2\bar{d}i)&=\min\{i+M,K\}\\
	k_{-}(2\bar{d}i)&=\max\{0,i-M\}
\end{align}
Note that
\begin{equation}
	a_{i}=
