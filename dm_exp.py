...
def Q(I,m,n,a,b):
    """[Q]uadrature"""
    return integrate.quad(lambda q: (q**m)*(I(q)**n),a,b)[0]
...
def D(I,N):
    ...
    #x1s = optimize.brentq(lambda x1: S(I,x1,N)[-1]-1.,1./N)
    #return S(I,x1s,N) 
    x0 = np.r_[1:N]/N
    x_star = optimize.fixed_point(lambda x: F(I,x),x0)
    x = np.zeros(N+1)
    x[1:-1] = x_star
    x[-1] = 1.
    return x
...
def F(I,x):
    """optimal discrete message function: fixed-point"""
    y = np.zeros(len(x))
    z = np.zeros(len(x)+2)
    z[1:-1] = x
    z[-1] = 1.
    for i in range(1,len(x)+1):
        y[i-1] = (A(I,z[i-1],z[i])+A(I,z[i],z[i+1]))/2.
    return y
...
class Message():
    ...
    def __init__(self,N,I,nplot=1000):
        ...
        #I_bar = integrate.quad(I,0,1)[0]
        self.N = N
        #self.I = lambda q: I(q)/I_bar
        self.I = I
        self.M = D(I,self.N) 
        self.nplot = nplot
...
def mplot(M,nplot=1000):
    ...
    plt.plot(q,cplt,'-r')
    for m in M:
        plt.plot(q,np.digitize(q,m.M)/(1.*m.N),'-b')
    plt.plot(q,q,'--k')
    plt.plot(q,cplt,'-r')
    plt.axis([0.,1.,0.,1.])
